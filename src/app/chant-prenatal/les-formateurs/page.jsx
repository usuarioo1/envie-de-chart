        ...
        <main className="px-4 py-12">
            <div className="mx-auto max-w-5xl space-y-10">
                ...
                <section className="rounded-3xl border border-[#F2B988] bg-white/85 p-8 shadow-[0_25px_60px_-40px_rgba(242,90,56,0.35)]">
                    {introHeading && (
                        <div className="flex items-center gap-3 pb-4 border-b-2 border-[#F29057]">
                            <svg className="h-6 w-6 text-[#F29057]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                            ...
                            <p className="text-sm font-semibold uppercase tracking-[0.3em] text-[#F29057]">{introHeading}</p>
                        </div>
                    )}
                    <div className="mt-8 grid gap-6 md:grid-cols-2">
                        {profiles.map((profile) => (
                            <article key={profile.title} className="group rounded-2xl border border-slate-100 bg-gradient-to-br from-white to-[#F2B988]/10 p-6 shadow-sm transition-all duration-300 hover:shadow-md hover:-translate-y-1">
                                <div className="flex items-start gap-3">
                                    <div className="flex-shrink-0 rounded-full bg-[#F2B988] p-2">
                                        <svg className="h-5 w-5 text-[#F25A38]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                                            <path d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" strokeLinecap="round" strokeLinejoin="round" />
                                        </svg>
                                    </div>
                                    <div className="flex-1">
                                        <h2 className="text-lg font-semibold text-slate-900 group-hover:text-[#F25A38] transition-colors">{profile.title}</h2>
                                        {profile.body && (
